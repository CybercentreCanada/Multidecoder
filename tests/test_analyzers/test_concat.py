...
def test_concat_empty_plus():
    test = b'""+""'
    assert find_concat(test) == [Hit(b"", ["concatenation"], 0, len(test))]
...
def test_concat_spacing():
    test = b'  "  "  +  "  "  '
    assert find_concat(test) == [Hit(b"    ", ["concatenation"], 2, len(test) - 2)]
...
def test_backslash_escape():
    ...
    assert find_concat(test) == [
        Hit(b'normal \\"nested\\" string', ["concatenation"], 0, len(test))
    ]
...
def test_concat_multiple_concat():
    ...
    assert find_concat(test) == [
        Hit(b"first second third", ["concatenation"], 0, len(test))
    ]
...
def test_concat_mixed_multiple():
    ...
    assert find_concat(test) == [
        Hit(b"doublesingledoublesingle", ["concatenation"], 0, len(test))
    ]
...
def test_concat_xml():
    ...
    assert find_concat(test) == [
        Hit(b"applesauce is a delicious food", ["concatenation"], 3, len(test) - 6)
    ]
