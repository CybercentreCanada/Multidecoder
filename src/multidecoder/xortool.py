...
def print_fitnesses(fitnesses):
    ...
    fmt = "{C_KEYLEN}{:" + str(len(str(max(i[0] for i in top10)))) + "d}{C_RESET}: {C_PROB}{:5.1f}%{C_RESET}"
...
def produce_plaintexts(ciphertext, keys, key_char_used):
    ...
    for index, key in enumerate(keys):
        ...
        if not PARAMETERS["filter_output"] or (PARAMETERS["filter_output"] and perc > threshold_valid):
            f = open(file_name, "wb")
            f.write(dexored)
            f.close()
    key_mapping.close()
